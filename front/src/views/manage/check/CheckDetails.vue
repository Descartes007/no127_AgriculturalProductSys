...
<!--        <el-button icon="el-icon-plus" circle @click="addStandard"></el-button>-->
      ...
      <el-form-item label="产品图片" label-width="150px">
        <img :src="baseApi + good.imgs" style="width: 170px;height: 150px">
<!--        <el-upload-->
<!--            class="upload-demo"-->
<!--            ref="upload"-->
...
<!--        </el-upload>-->
      ...
      <el-form-item label-width="150px">
        <el-button type="success" @click="returnList" style="font-size: 20px;">
          返回审核列表</el-button>
      </el-form-item>
    </el-form>
    <!-- 弹窗   -->
...
export default {
  ...
  methods: {
    ...
    //保存数据
    save() {
      this.good.userId = this.user.id;
      // 先保存检测列表信息，再保存规格信息
      API.post(url, this.good).then((res2) => {
        if (res2.code === "200") {
          this.good.id = res2.data.id;
          this.request
              .post(url + "/standard?checkId=" + this.good.id, this.standards)
              .then((res) => {
                if (res.code === "200") {
                  this.$message({
                    type: "success",
